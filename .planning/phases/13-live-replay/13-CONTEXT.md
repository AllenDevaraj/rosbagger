...
- **D-05 — Raw-CDR source (RESEARCH to confirm the cleanest path):** the recommended default is to read raw CDR + topic + type + timestamp via **`rosbag2_py.SequentialReader`** on the live side (mirrors `rosbagger-record`'s `rosbag2_py` usage and yields raw bytes natively). The v1 `RosbagsReader` (Phase 2 dependency) remains the canonical reader for bag metadata/validation and for the SC re-open checks; research decides whether the live read path uses `rosbag2_py.SequentialReader` directly or a small raw-bytes seam added to the v1 reader. Either way the publish bytes must be raw CDR, not a rosbags-deserialized object.
