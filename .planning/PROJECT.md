...
- [x] v0.2 packages are git/path-installable into other repos with coherent versioning + central install docs — *Validated in Phase 15 (DoD): all five packages stamped 0.2.0 (10 sites) with sibling deps pinned `rosbagger-core>=0.2,<0.3` (version spec only — no URL baked into `[project] dependencies`, D-01), `rosbagger-gui[live]` declarative extra (D-02), re-locked `uv.lock`. `scripts/proof_external_install.sh` proves external spec resolution: builds + co-installs all five by local path into a fresh venv outside the monorepo (one transaction) → `rosbagger-gui --help` / `bagq --version` / five imports / offline invariant / `tools`-not-in-wheel all green (`PROOF OK`). `INSTALL.md` documents the one-transaction git+subdirectory meta recipe, per-package snippets, path install, consumer-side `[tool.uv.sources]`, and the `[live]` note. Release gate green (`uv build --all-packages` → 5 wheels, `uv sync --locked`, ruff, 466 ROS-free tests @97.37% incl. the offline guard under the live extra); local annotated `v0.2.0` tag created. Remote push + literal git recipe = documented human follow-up (standing no-push blocker; Phase 8 honest-split precedent). Code review 0 blocker + 4 warning + 2 info (advisory, `15-REVIEW.md`/`15-VERIFICATION.md`).*
- [x] A native desktop GUI cockpit (PySide6/Qt): five capability-gated panels with full parity to the TUI, reusing the module APIs verbatim — *Validated in Phase 16 (milestone v0.3 DoD): new isolated `rosbagger-desktop` package (PySide6) — `rosbagger-desktop [BAG]` spawns a `QMainWindow` cockpit (nav + `QStackedWidget`, App-owned shared `RosbagsReader`, tier-1 rclpy capability gate, File ▸ Open via `QFileDialog`) hosting five thin-face panels: inspect (`collect_bag_info`+`collect_table_schemas`), query (`query`+`write_table`+schema tree + save-dialog export + teaching errors), tf (`collect_tf_report`), live record (`list_record_topics`/`record_topics` on QThread workers), live replay (pure `Replayer` over the SHARED `build_publish_sink` — single publish path — + a Qt `Scrubber` + `list_events` markers). Hard isolation held: PySide6 confined to this one package, ROS imports lazy, the existing TUI (`rosbagger-gui`) and all other package source UNTOUCHED, and the offline import graph stays ROS-free AND Qt-free (offline guard extended with PySide6/shiboken6 blocklist). Proven headless via pytest-qt `QT_QPA_PLATFORM=offscreen` against fixtures; live record/replay on the `@pytest.mark.live` lane. 485 passed / 4 skipped offline tests at 97.37%; code review 2 critical + 6 warning all FIXED (with regression tests), 4 info tracked (`16-REVIEW.md`/`16-VERIFICATION.md`). The TUI coexists untouched.*
- [x] A Textual GUI cockpit: five capability-gated panels (record/inspect/query/tf/replay) over the existing module APIs — *Validated in Phase 14 (GUI-01): new `rosbagger-gui` package (the first GUI module) — a Textual App shell (sidebar `ListView` + `ContentSwitcher`, App-owned shared `RosbagsReader`, cheap tier-1 `rclpy`-availability gate that disables the live panels with a teaching hint) hosting five thin-face panels: inspect (`collect_bag_info`+`collect_table_schemas`), tf (`collect_tf_report`), query (`query`/`write_table`/`collect_table_schemas` + schema tree + history + CSV/Parquet export), live record (`discover_topics`/`record_topics` in `@work(thread=True)` workers), live replay (pure `Replayer` over the SHARED `build_publish_sink` from 14-01 — single publish path — + a custom scrubber widget + `list_events` markers). Every panel holds zero analysis logic and lazy-imports ROS inside method bodies only; the package imports ROS-free (offline-guard extended with `test_import_gui_does_not_pull_ros`). SC1/SC2/SC3 proven headless via `App.run_test()`/`Pilot` against a `make_fixtures` bag, ROS-free in CI; live record/replay covered on a `@pytest.mark.live` ROS-sourced lane. 465 ROS-free offline tests at 97.37%; code review 0 critical, 6 warnings + 6 info (advisory — logged in `14-REVIEW.md`).*

### Active

<!-- v0.2 "Modular cockpit" (Phases 9–15): COMPLETE. -->

- *No active (unvalidated) requirements. v0.2 "Modular cockpit" milestone is **complete** (Phases 9–15). Milestone **v0.3 "Desktop cockpit"** opened and Phase 16 (Native Desktop GUI, PySide6) is **complete + validated above** — a native Qt desktop cockpit now coexists with the Textual TUI, both thin frontends over the same module APIs. The offline/live boundary still holds end-to-end: `rclpy` is isolated to `rosbagger-record` + `rosbagger-replay` (+ lazy in both GUIs' live panels) and absent from the uv venv; `rosbagger_core` / `bagq` / the `rosbagger-gui` (TUI) / `rosbagger-desktop` (Qt) import graphs stay ROS-free, and the desktop package additionally keeps the offline graph Qt-free.*
...
---
*Last updated: 2026-05-25 — **v0.3 "Desktop cockpit" — Phase 16 (Native Desktop GUI, PySide6) shipped: rosbagger's first NATIVE desktop window.** New isolated `rosbagger-desktop` package — `rosbagger-desktop [BAG]` spawns a `QMainWindow` cockpit (nav + `QStackedWidget`, App-owned shared reader, rclpy capability gate, `QFileDialog` open) with FULL parity to the Textual TUI's five panels, each a thin face calling the existing `rosbagger_core`/`rosbagger_record`/`rosbagger_replay` APIs verbatim — zero new analysis logic. Live panels run blocking work on `QThread`/`QObject` workers; replay drives the pure `Replayer` through the SHARED `build_publish_sink` (single publish path). Hard isolation constraint met: PySide6 confined to the new package, ROS imports lazy, the existing TUI + all other source UNTOUCHED, and the offline import graph kept ROS-free AND Qt-free (guard extended). Headless pytest-qt (`QT_QPA_PLATFORM=offscreen`); 485 passed / 4 skipped at 97.37%; code review 2 critical + 6 warning ALL FIXED with regression tests (4 info tracked) — `16-REVIEW.md`/`16-VERIFICATION.md`. The TUI and the native GUI now coexist. 1 phase done in v0.3; 16 phases done overall. Human follow-up: sanity-check the live-ROS replay path on the `-m live` lane.*

*(prior) Last updated: 2026-05-24 — **v0.2 "Modular cockpit" RELEASED — Phase 15 (Packaging & Release) shipped: all five packages are now git/path-installable at a coherent v0.2.0.** Mirrors Phase 8's v0.1 release work for the four packages it didn't cover. 10 version sites stamped 0.2.0; the four sibling deps pinned `rosbagger-core>=0.2,<0.3` by version spec only (no URL in `[project] dependencies` — D-01); `rosbagger-gui` gained the declarative `[live]` extra (D-02); `uv.lock` re-locked. The wheel-resolution gap is closed at the source and proven by `scripts/proof_external_install.sh` — a fresh venv outside the monorepo co-installs all five by path in one transaction (the bare sibling spec resolves in-transaction) and passes six smoke checks → `PROOF OK`. `INSTALL.md` documents every install path (git+subdirectory meta recipe, per-package, path, consumer-side `[tool.uv.sources]`, `[live]` note). Release gate green: `uv build --all-packages` → five 0.2.0 wheels, `uv sync --locked`, ruff, 466 ROS-free tests @97.37% incl. the offline guard surviving the live extra (SC5); local annotated `v0.2.0` tag created (push = sole human follow-up, standing no-push blocker). Code review 0 blocker + 4 warning + 2 info (advisory, `15-REVIEW.md`/`15-VERIFICATION.md`). 7 phases done in v0.2; 15 phases done overall — milestone complete and shippable.*

*(prior) Last updated: 2026-05-24 — **v0.2 "Modular cockpit" COMPLETE — Phase 14 (GUI) shipped: rosbagger's first GUI module.** `rosbagger-gui` is a Textual cockpit of five capability-gated panels (record/inspect/query/tf/replay), each a thin face over the existing module APIs against an App-owned shared reader. The replay panel drives the pure `Replayer` through the SHARED `build_publish_sink` (extracted in 14-01) — exactly one production publish path. The offline guarantee held: every ROS symbol is lazy-imported inside method bodies, the live panels are gated off when `rclpy` is absent, and the package import graph is ROS-free (offline-guard extended). SC1/SC2/SC3 proven headless via `App.run_test()`/`Pilot` against a `make_fixtures` bag (ROS-free in CI); record/replay covered on a `@pytest.mark.live` lane. 465 ROS-free offline tests at 97.37%; code review 0 critical + 6 warnings + 6 info (advisory, `14-REVIEW.md`/`14-VERIFICATION.md`). This is the LAST v0.2 phase — milestone complete: 7 phases done in v0.2; 14 phases done overall.*
