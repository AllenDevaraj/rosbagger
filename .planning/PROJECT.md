...
- *v0.2 "Modular cockpit" milestone in progress — Phases 9 (TF debugger) + 10 (query ergonomics) + 11 (edit & events) + 12 (live record) ✓ validated above; Phases 13–14 pending (live replay needs `rclpy`; GUI's offline panels don't). The offline/live boundary held through the first live phase: `rclpy` is isolated to `rosbagger-record` and absent from the uv venv (env-provided by sourced ROS).*
