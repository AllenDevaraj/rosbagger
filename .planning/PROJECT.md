...
- *No active (unvalidated) requirements. v0.2 "Modular cockpit" milestone is **complete** — Phases 9 (TF debugger) + 10 (query ergonomics) + 11 (edit & events) + 12 (live record) + 13 (live replay) + 14 (GUI) + 15 (packaging & release) all validated above. The offline/live boundary held end-to-end: `rclpy` is isolated to `rosbagger-record` + `rosbagger-replay` (+ lazy in the GUI's live panels) and absent from the uv venv (env-provided by sourced ROS); `rosbagger_core` / `bagq` / the `rosbagger-gui` import graph stay ROS-free.*
